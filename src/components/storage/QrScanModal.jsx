...
// changes — like a real barcode scanner's momentary "beep and flash" before
// it acts, not a confirmation step the user has to tap through.
...
export function QrScanModal({ boxes, onClose, onFound }) {
  ...
  // Modal hands `requestClose` to the render-prop body below, not to this
  // component directly — stashed in a ref so the auto-advance effect (which
  // needs to fire outside of a render) can still reach it.
  ...
  // Recognizing a box jumps straight there — no "View box" tap required.
  useEffect(() => {
    if (!foundBox) return undefined;
    const timer = setTimeout(() => {
      requestCloseRef.current?.(() => onFound(foundBox));
    }, FOUND_DISPLAY_MS);
                ...
                >
                  ...
                  {foundBox && (
                    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <BoxQrCode value={boxDeepLinkUrl(foundBox.number)} label={formatBoxNumber(foundBox.number)} size={110} />
                    </div>
                  )}
                </div>
