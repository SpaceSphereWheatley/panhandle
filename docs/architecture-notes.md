...
`public/changelog.html` fetches `/CHANGELOG.md` and renders every version, but — as of 1.51.7 — only each bullet's lead sentence, same as `ChangelogModal`; it's the "full changelog" destination (full *history*, not full *detail* per entry) for both the landing page footer and `ChangelogModal`'s "See the full changelog" link. This is deliberately a same-origin static page rather than a link out to GitHub: the landing page is the product's public face, and routing off-domain to GitHub would leak an implementation detail (and break if the repo ever went private). `ChangelogModal` shows the same compact per-version titles (`src/lib/changelogUtils.js`'s `parseChangelog`/`extractTitle`) for just the last few releases; `/changelog.html` shows every release but still only each entry's lead sentence, via its own hand-rolled markdown parser (not `parseChangelog`, since `public/` has no build step and can't import from `src/`) that reimplements the same lead-sentence-extraction rule, tailored to this file's actual shapes (headings, one level of nested bullets, wrapped continuation lines) rather than being general-purpose. `CHANGELOG.md` itself keeps the fuller technical explanation after each entry's lead sentence — neither surface shows it, but it stays in the source file for developer/git-history reference. Every bullet is written so its first sentence alone reads as a plain-language, non-technical summary; the technical continuation is not deleted, just never rendered to a user.
