...
108. (15) English-first restructure — the codebase's source language flipped
     ...
     `shared/errorCodes.js` holds canonical English text; `CATEGORIES` and all
     710 `COMMON_ITEMS` names are English strings; the seven Norwegian-named
     Settings subpages, their route slugs and their dictionary key namespaces
     are renamed. Norwegian stays the app's **default UI language**
     (`language.js`'s `DEFAULT_UI_LANGUAGE`) — that constant and
     `translate.js`'s `DEFAULT_LANG` (the base dictionary, now English) are
     deliberately different things, and each documents which role it plays.

     Two data migrations (0022, 0023) rewrite the stored canonical values.
     Unlike every previous migration here these are **not** expand/contract —
     they rewrite values live code matches on literally — so they were applied
     immediately before the merge rather than ahead of it, accepting a ~1
     minute window where the old code saw renamed data. 0023 is mandatory
     rather than cleanup: `checkCatalogueSync` re-seeds on a `COMMON_ITEMS`
     hash change, so without it every list would gain 710 duplicate rows.

     Two Norwegian names shared an English translation
     (Grandiosa/Frossenpizza, Polarbrød/Flatbrød) and would have violated
     `UNIQUE(list_id, name)`; both keep their brand name as the canonical
     English one. The three 710-entry name lists were regenerated by
     `scripts/flip-item-names-to-english.mjs` rather than hand-edited, and a
     new test asserts they cover exactly the same set — a partial rename would
     otherwise surface only as a silently missing icon.

     Also converted 29 integration-test assertions from matching Norwegian
     error prose to asserting the stable `code`.
