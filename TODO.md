...
17. Restructure user identity around three editable fields — **Name** (display
    name, shown in the UI), **e-mail**, and **username**, where username must
    always equal the e-mail — with Google-linked accounts taking Name/e-mail
    from Google, and existing accounts migrated onto the new invariant. Today
    there is no `name` column at all: the UI shows raw `username` everywhere
    a person's identity appears (`ProfileIsland`'s "Innlogget som",
    `MembersIsland`, the meal-responsible dropdowns, "lagt til av" on items).
    `POST /auth/google` (`worker/index.js`) also only reads
    `payload.email`/`payload.sub` off the Google ID token today, never
    `payload.name`. Real blockers to work through, not just a form change:
    - `users.username` is a `TEXT PRIMARY KEY` copied *by value* — not by
      foreign key — into `list_items.added_by`, `meal_plan.responsible`,
      `users.created_by`, and `password_resets.username`, and it's also the
      JWT `sub`. Renaming a user's username (to backfill the "username =
      email" invariant, or on a later e-mail change) means a coordinated
      multi-table update plus bumping `token_version` to force every device
      onto a fresh token, not a single-column `UPDATE users`.
    - Enforcing "username = email" needs every account to have an e-mail.
      That's not true today: `POST /admin/owners` and `POST /list-users`
      deliberately create password-only members with no e-mail (a household
      member who shares the list without their own inbox — see CLAUDE.md's
      Auth model). Needs a product decision — require e-mail at creation for
      those flows too, or keep them as a permanent exception to the rule?
    - Migrating the *existing* users who already have an e-mail on file is a
      cascading rename (see above). Migrating the ones with no e-mail at all
      has nothing to rename to — needs either a forced "add your e-mail"
      prompt on next login or an admin-driven cleanup pass; the DB migration
      alone can't finish this part.
    - New `PATCH /account`-style endpoint (or extend `/change-email`) plus a
      Name field/input in `ProfileIsland.jsx` (Settings) — only e-mail and
      password are editable there today.
    - For a Google-linked account, decide whether Google's Name/e-mail always
      overwrite a local edit on every sign-in (simplest, but removes the
      user's ability to show a different display name than their Google
      profile) or only seed the fields once and leave them user-editable
      after.
    _Value: Medium · Importance: Medium · Type: Data model / Auth / Settings_
