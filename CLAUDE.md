...
The frontend is mid-migration from a hand-rolled, no-build vanilla JS/HTML app to a Vite + React build. A Vite + React port of `public/app.html` lives at the repo root (`package.json`, `vite.config.js`, root `app.html`, `src/` — see Architecture below); all three tabs are implemented and have been manually verified against the real API. **It is not yet wired into the live deploy**: the Cloudflare Pages dashboard still has no build command configured and still deploys `public/` as-is, so `public/index.html`/`public/app.html` remain the actual live frontend until the dashboard settings are deliberately switched over (build command → `npm run build`, output directory → `dist`). Until that cutover, keep changing the live frontend in `public/`, not `src/`.
...
- **`public/`** — the **live** frontend served by Cloudflare Pages, no build step: `index.html` is a static marketing/landing page (unauthenticated, links to `/app.html`), `app.html` is the actual single-file app (login + all three tabs, inline `<style>`/`<script>`), `seed.html` (one-time account creation form, posts to `/seed`), `manifest.json`, icons. Pages build output directory is currently `public/`, with no build command configured.
- **`src/`, `package.json`, `vite.config.js`, root `app.html`** — a Vite + React port of `public/app.html` (all three tabs — shopping list, meals, settings — are implemented; see `src/tabs/`). Not yet connected to the Pages deploy (no dashboard build command points at it). The Vite entry is named `app.html` (not `index.html`) specifically so it builds to `dist/app.html`, matching the real app's URL and leaving `public/index.html`'s copy-through (via Vite's `publicDir`) untouched at `dist/index.html` instead of colliding with it.
