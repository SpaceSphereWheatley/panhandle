...
- Phase 1 of TODO #7: Web Push subscribe/unsubscribe infrastructure plus a single notification type, a cron-driven "no meal planned for tomorrow" reminder. Batched item-added notifications, a weekly meal-plan reminder, and a custom "get the other person's attention" ping are later phases of the same item, not built yet.
- `migrations/0012_push_notifications.sql` adds three tables. `push_subscriptions` is keyed by `endpoint` (not `username`) since a subscription belongs to a browser/device, not an account — on a shared household device, whoever last (re)subscribed owns it; `POST /push/subscribe` upserts `ON CONFLICT(endpoint)`. `notification_settings` holds one row per list (`meal_reminder_enabled`, `meal_reminder_time` as `HH:mm`, 15-minute increments only) — no row is seeded, so `GET /notification-settings` falls back to app-level defaults (`enabled`, `18:00`) until a list's first `POST`. `notification_log` (`UNIQUE(list_id, type, target_date)`) is a dedup guard so the cron never sends the same list/day's reminder twice.
- Endpoints (all behind the auth boundary — a user must already be logged in to reach the Settings control, so there's no new public route): `GET /push/vapid-public-key`, `POST`/`DELETE /push/subscribe`, `GET`/`POST /notification-settings` (same any-list-member permission level as `/recurring`, not owner-gated). A departing/removed member's `push_subscriptions` row is cleaned up in `DELETE /account`, `DELETE /admin/users/{u}`, and `DELETE /list-users/{u}` alongside the existing per-list cascades, since it's a live credential, not historical data.
- `wrangler.toml`'s `[triggers]` cron (`*/15 * * * *`) drives `worker/index.js`'s exported `scheduled(event, env, ctx)`, a thin `ctx.waitUntil(runReminderPass(env, event.scheduledTime))` wrapper. `runReminderPass(env, nowMs)` takes `now` as a parameter (rather than reading `Date.now()` internally) specifically so `tests/push-notifications.test.mjs` can exercise its dedup/skip/fan-out logic deterministically against a fake `env.DB` stub, without depending on wrangler dev's untested `--test-scheduled` scheduled-event simulation. `osloLocalDateParts`/`isReminderDue` (also exported, unit-tested in `tests/worker-unit.test.mjs`) compute the current Europe/Oslo local time and "today"/"tomorrow" via `Intl.DateTimeFormat`, which correctly follows Oslo's CET/CEST DST transitions; two known, accepted edge cases (documented in code, not fixed): a reminder time inside the skipped spring-forward hour silently doesn't fire that one day/year, and the repeated fall-back hour would double-fire if `notification_log`'s dedup constraint didn't absorb it as a no-op.
...
- Frontend: `src/context/PushContext.jsx` holds subscribe/unsubscribe state and re-syncs the browser's current subscription to the server on every mount/token change (also how a subscription silently rotated while no tab was open gets its new endpoint picked up — see below). `src/components/settings/NotificationsIsland.jsx` (Settings → "Varsler") has the enable-notifications toggle and the shared reminder-enabled/time controls. `public/sw.js` gained `push` (shows the notification), `notificationclick` (focuses/opens the app), and `pushsubscriptionchange` (browsers can silently rotate/invalidate a subscription outside of any 404/410 the server would see) listeners — the last of these only keeps the *browser-level* registration alive, since a service worker has no access to this app's auth token (it lives in `localStorage`, unreachable from a SW) to re-authenticate a server-side sync itself; that sync happens next time the app opens, via `PushContext`'s refresh.
- Web Push on iOS only works for the PWA added to the Home Screen (standalone display mode), not an ordinary Safari tab, and only iOS 16.4+ — `NotificationsIsland` shows a hint when it detects that case.
